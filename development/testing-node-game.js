  ...
  for (var key in words) {
    ...
    if (words.hasOwnProperty(key)) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        countHash[letter] ? countHash[letter]++ :
          countHash[letter] = 1;
      }
    }
  }
...
function letterGuesser(wordsHash, length,
...
function handleWord(msg, word){
  ...
  if(isNewWord) saveWord(wordToSave);
}

function saveWord(word){
  isNewWord = false;

  let path = `./dictionary-json/${word.length}-letter.json`;
  let wordArr = word.split('');
  let jsonDictionary = require(path);
  let nextWordKey = Object.keys(jsonDictionary).length;

  jsonDictionary[nextWordKey] = wordArr;
